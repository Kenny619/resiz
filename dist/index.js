...
class Resiz {
    ...
    async run(option) {
        if ("width" in option && typeof option.width === "number")
            this.#newWidth = option.width;
        if ("height" in option && typeof option.height === "number")
            this.#newHeight = option.height;
        if ("source" in option && typeof option.source === "string")
            this.#source = option.source;
        if ("destination" in option && typeof option.destination === "string")
            this.#dstDir = option.destination;
        if ("quality" in option && typeof option.quality === "number")
    ...
    async #validateInputs() {
        ...
        if (!this.#source)
            throw new Error("Specify the source fle or directory.");
        //set srcFile/srcDir
        try {
            const stat = await fsp.stat(path.resolve(process.cwd(), this.#source));
            if (stat.isFile())
                await this.#setSrcFile();
            else if (stat.isDirectory())
                await this.#setSrcDir();
        }
        catch (e) {
        ...
        if (this.#quality < 0 || this.#quality >= 100)
            throw new Error(`Invalid quality: ${this.#quality}`);
        //set destination
        try {
    ...
    async #setDstDir() {
        if (!this.#dstDir) {
            ...
            if (this.#srcFile)
                this.#dstDir = path.join(path.dirname(this.#srcFile), "..", dirName);
            if (this.#srcDir)
                this.#dstDir = path.join(this.#srcDir, "..", dirName);
        }
        try {
    ...
    async resizeSingleFile(filePath) {
        ...
        outputInstance
            ...
            .toFile(path.join(dstDir, filename));
    }
}
